...
TF_VAR_S3_BUCKET_NAME="chaitalithreetierbucket"
RED='\033[0;31m'
...
aws --profile ckanetkar s3 ls "s3://${TF_VAR_S3_BUCKET_NAME}"
...
if [[ ${exit_val} -eq 0 ]]; then
  ...
  aws --profile ckanetkar s3api create-bucket --bucket ${TF_VAR_S3_BUCKET_NAME} --region ap-south-1 --create-bucket-configuration LocationConstraint=ap-south-1
fi

aws --profile ckanetkar s3 cp "./s3content" s3://${TF_VAR_S3_BUCKET_NAME}/ --recursive
